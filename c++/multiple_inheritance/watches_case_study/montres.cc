...
public:
    ...
    coeur(autre.coeur->copie())
  {
    for (auto const& p_acc : autre.accessoires) {
      // permet de copier chaque accessoire de l'autre montre de façon polymorphique
      accessoires.push_back(p_acc->copie());
      // copie() est const = 0 au niveau de Accessoire, mais overridden au niveau des sous classes
    }
  }

  Montre& operator=(Montre source)
  // Notez le passage par VALEUR
  // => utilise le constructeur de copie qui se charge de faire la copie profonde
  {
